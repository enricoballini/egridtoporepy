function [deck,G] = sinusDeckAdiVE(cartDims, physDims, nsteps, dt, theta,...
                                 depth, phi, perm, rate, p_press)
%Make a GRDECL structure for simple sloping sinus-formed reservoir.
%
% SYNOPSIS:
%   [deck,G] = sinusDeckAdiVE(n, L, nt, dt, theta, depth, phi, K, r, p)
%
% PARAMETERS:
...
deck.PROPS.SWOF{1} = [s, s, 1-s, ((1-s)*drho*norm(gravity)*physDims(3))/barsa];
...
G = computeGeometry(G);
deck.SOLUTION.PRESSURE = convertFrom(...
   p_press + norm(gravity)*deck.PROPS.DENSITY(2)*G.cells.centroids(:,3),...
   barsa);
...
deck.SCHEDULE.step.val     = deck.SCHEDULE.step.val;
