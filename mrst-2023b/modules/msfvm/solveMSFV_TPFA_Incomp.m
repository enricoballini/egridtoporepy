...
%                 U = gmres(@(u) A*u,r,opt.Restart,10e-9,opt.Iterations,invG, [], U);
    ...
    if opt.Reconstruct
        ...
        sp = DG.P_flux'*mldivide(D, DG.P_flux*state.rhs(1:Nf) - (Abar - D)*DG.P_flux*p0);
    ...
    state = setFlux(G, vertcat(p0, tmp), T, fluid, state, opt);
    ...
    if opt.Reconstruct
        ...
        state_o = setFlux(G, vertcat(p, tmp), T, fluid, state, opt);


        % sample the fluxes from the original pressure over coarse
        % boundaries
        flux = state_o.flux;
