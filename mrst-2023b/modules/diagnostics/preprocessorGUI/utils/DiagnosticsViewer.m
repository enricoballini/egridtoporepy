classdef DiagnosticsViewer < handle
    ...
    methods
        function d = DiagnosticsViewer(arg1,arg2,varargin)
            ...
            % only consider filtering for props with non-empty limits
            filterProps = d.Props;
            nonEmptyLims = ~cellfun(@isempty, filterProps.diagnostics.limits);
            filterProps.diagnostics.name = filterProps.diagnostics.name(nonEmptyLims);
            filterProps.diagnostics.limits = filterProps.diagnostics.limits(nonEmptyLims);
            selector3D.fsel = PropertyDisplaySelector( ...
                'Title', 'Filter display', 'includeFilter', true, ...
                'props', filterProps, 'includePlayer', false, ...
                itemOpts{:},...
