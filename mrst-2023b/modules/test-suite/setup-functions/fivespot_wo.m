function setup = fivespot_wo(varargin)
% Setup function for a five-spot water-oil model
%
% SYNOPSIS:
%   setup = fivespot_wo('pn1', pv1, ...)
%   setup = fivespot_wo(fullSetup, 'pn1', pv1, ...)
%
% DESCRIPTION:
%   Setup of a standard five-spot case for an immisicble
...
%      'pvi'    - total pore volumes injected      (default: 1)
...
%
% SEE ALSO:
%   TestCase, testcase_template, testSuiteTutorial, qfs_wo.
...
%}
    ...
    description = 'Five-spot example, with two-phase fluid on Cartesian grid';
    
    % Optional input arguments
    options = struct('ncells', 51    , ... % Number of cells in x- and y-directions
    ...
    % Wells
    rate = options.pvi*sum(poreVolume(G, rock))/(4*options.time);
    bhp  = 50*barsa;
    dim = G.cartDims(1:2);
    pos = ceil(dim./2);
    W = [];
    W = verticalWell(W, G, rock, pos(1), pos(2), [], 'type', 'bhp', 'val', bhp, 'comp_i', [1,0], 'sign', -1);
    W = verticalWell(W, G, rock, 1,      1,      [], 'type', 'rate', 'val', rate, 'comp_i', [1,0], 'sign', 1);
    W = verticalWell(W, G, rock, 1,      dim(2), [], 'type', 'rate', 'val', rate, 'comp_i', [1,0], 'sign', 1);
    W = verticalWell(W, G, rock, dim(1), 1,      [], 'type', 'rate', 'val', rate, 'comp_i', [1,0], 'sign', 1);
    W = verticalWell(W, G, rock, dim(1), dim(2), [], 'type', 'rate', 'val', rate, 'comp_i', [1,0], 'sign', 1);
