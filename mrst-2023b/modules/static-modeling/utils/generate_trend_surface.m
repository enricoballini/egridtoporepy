...
%   num_coefs - Number of knot intervals in x and y coordinates (u and v parameters)
...
%}
   ...
   if isscalar(num_coefs)
       num_coefs = [num_coefs, num_coefs];
   end

   % fitting trend surface
   surf = BivariateSplineFunction([4, 4], num_coefs(1), num_coefs(2));
   ...
   x = linspace(box(1), box(3), samples(1));
   y = linspace(box(2), box(4), samples(2));
   
   [U, V] = ndgrid(x, y);

   zgrid = reshape(surf.evaluate([U(:), V(:)], 0, 0), samples(1), samples(2));
end
