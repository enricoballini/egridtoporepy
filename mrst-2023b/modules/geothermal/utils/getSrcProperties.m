function src = getSrcProperties(src, model, state)
%Compute properties at the boundary

    propsRes = getReservoirProperties(model, state, src);
    propsBC  = getBoundaryProperties(model, propsRes, src);
    src.propsRes = propsRes;
    src.propsBC  = propsBC;
    src.sat = value(src.propsBC.s);
    % Hack to get correct relperm dimensions when nph = 1
    model.G.cells.num = numel(src.face);
...
%-------------------------------------------------------------------------%
function props  = getReservoirProperties(model, state, src)
